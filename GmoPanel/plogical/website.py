...
class Website:
    ...
    def activeTheme(self, data=None):
        ...
        try:
            ...
            for line in file:
                if re.search(r'\$table_prefix', line):
    ...
    def migration_wp(self, domain=None):
        ...
        try:
            ...
            for line in fread:
                if re.search(r"define\('DB_NAME',", line):
                    fwrite.write("define('DB_NAME', '{}');".format(db_name))
                elif re.search(r"define\('DB_USER'", line):
                    fwrite.write("define('DB_USER', '');".format(db_user))
                elif re.search(r"define\('DB_PASSWORD'", line):
                    fwrite.write("define('DB_PASSWORD', '');".format(db_pass))
                elif re.search(r"define\('DB_HOST',", line):
            ...
            cache_set.b_cache('on')
            cache_set.f_cache('on')
